...
function void
generic_parse_init(App *app, Arena *arena, String_Const_u8 contents, Token_Array *tokens, Generic_Parse_Comment_Function *handle_comment, Generic_Parse_State *state){
    state->app = app;
    state->arena = arena;
...
cpp_parse_type_def(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
    ...
    for (;;) {
        ...
        } else if (token->kind == TokenBaseKind_StatementClose || token->kind == TokenBaseKind_ScopeOpen || token->kind == TokenBaseKind_ScopeClose || token->kind == TokenBaseKind_ScopeOpen || token->kind == TokenBaseKind_ScopeClose) {
            break;
        } else if (token->kind == TokenBaseKind_Keyword) {
            String_Const_u8 lexeme = string_substring(state->contents, Ii64(token));
            if (string_match(lexeme, string_u8_litexpr("struct")) || string_match(lexeme, string_u8_litexpr("union")) || string_match(lexeme, string_u8_litexpr("enum"))) {
                break;
            }
        }
        if (!did_advance) {
...
function void cpp_parse_function(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent) {
    Token *token = token_it_read(&state->it);
    generic_parse_inc(state);
    generic_parse_skip_soft_tokens(index, state);
    if (state->finished) {
...
function Code_Index_Nest *generic_parse_preprocessor(Code_Index_File *index, Generic_Parse_State *state);
function Code_Index_Nest *generic_parse_scope(Code_Index_File *index, Generic_Parse_State *state);
function Code_Index_Nest *generic_parse_paren(Code_Index_File *index, Generic_Parse_State *state);
...
function Code_Index_Nest *generic_parse_statement(Code_Index_File *index, Generic_Parse_State *state) {
    ...
    for (;;) {
        ...
        if (token->kind == TokenBaseKind_ScopeOpen || token->kind == TokenBaseKind_ScopeClose || token->kind == TokenBaseKind_ParentheticalOpen) {
            result->is_closed = true;
            result->close     = Ii64(token->pos);
            break;
        }
    ...
    for (;;) {
        ...
        if (token->kind == TokenBaseKind_Preprocessor) {
            ...
            if (token->sub_kind == TokenCppKind_Struct || token->sub_kind == TokenCppKind_Union || token->sub_kind == TokenCppKind_Enum) {
                ...
                // TODO(Krzosa): Need to decide what indentation is better
                // TODO(Krzosa): If keep no indentation then need to fix auto_indent
                generic_parse_inc(state);
                generic_parse_skip_soft_tokens(index, state);
                Token *token = token_it_read(&state->it);
                if (token->kind == TokenBaseKind_Identifier) {
...
function void
default_comment_index(App *app, Arena *arena, Code_Index_File *index, Token *token, String_Const_u8 contents){

}

function void
generic_parse_init(App *app, Arena *arena, String_Const_u8 contents, Token_Array *tokens, Generic_Parse_State *state){
...

        {
            for (; ptr < end_ptr; ptr += 1) {
                ...
                if (*ptr == '\r') {
                    ...
                    i64                  index = layout_index_from_ptr(ptr, text.str, range.first);
                    Token_Iterator_Array it    = token_iterator_pos(0, &tokens, index);
                    Token *              b     = token_it_read(&it);
                    if (b->kind == TokenBaseKind_LiteralString || b->kind == TokenBaseKind_Comment) {
                        lr_tb_advance_x_without_item(&pos_vars, metrics.space_advance);
                    }
                    //     i64 index = layout_index_from_ptr(ptr, text.str, range.first);
    ...
    consuming_normal_whitespace:
        for (; ptr < end_ptr; ptr += 1) {
            if (!character_is_whitespace(*ptr)) {
                ...
                for (Code_Index_Nest *nest = new_wrap_nest; nest != 0; nest = nest->parent) {
                ...
                if (new_wrap_token_pair.a.kind == TokenBaseKind_Keyword) {
                    if (new_wrap_token_pair.b.kind == TokenBaseKind_ParentheticalOpen || new_wrap_token_pair.b.kind == TokenBaseKind_Keyword) {
                        token_score -= 2;
                    }
                }
