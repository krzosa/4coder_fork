...
function void
clipboard_collection_render(App *app, Frame_Info frame_info, View_ID view){
    Scratch_Block scratch(app);
...
function Range_i64
multi_paste_range(App *app, View_ID view, Range_i64 range, i32 paste_count, b32 old_to_new){
    Scratch_Block scratch(app);
...
function void
multi_paste_interactive_up_down(App *app, i32 paste_count, i32 clip_count){
    View_ID view = get_active_view(app, Access_ReadWriteVisible);
    i64 pos = view_get_cursor_pos(app, view);
    b32 old_to_new = true;
...
function void
clipboard_clear(App *app, i32 clipboard_id){
    clipboard_clear(clipboard_id);
}
function b32
clipboard_post(App *app, i32 clipboard_id, String_Const_u8 string){
    return(clipboard_post(clipboard_id, string));
}
function i32
clipboard_count(App *app, i32 clipboard_id){
